...
import { X, CheckCircle, XCircle, Edit, Trash2, MapPin, Plus } from "lucide-react";
      ...
      <>
        ...
        <motion.div className="admin-drawer" initial={{ x: "100%" }} animate={{ x: 0 }} exit={{ x: "100%" }} transition={{ type: "spring", damping: 28, stiffness: 300 }}>
          <div className="admin-drawer-header">
            ...
            <button className="admin-btn admin-btn-secondary admin-btn-icon" onClick={onClose}><X size={16} /></button>
          </div>
        </motion.div>
      </>
    );
    ...
    <>
      ...
      >
        ...
        <div className="admin-drawer-body" style={{ overflowY: "auto" }}>
          ...
          {/* Services Editor */}
          <div className="admin-card" style={{ padding: 16 }}>
            <div className="admin-card-title" style={{ marginBottom: 12 }}>✂️ Services</div>
            <div style={{ display: "flex", flexDirection: "column", gap: 8 }}>
              ...
              <div style={{ display: "grid", gridTemplateColumns: "1fr 80px 80px auto", gap: 6, marginTop: 4 }}>
                ...
                <button className="admin-btn admin-btn-success admin-btn-sm" onClick={handleAddService}><Plus size={12} /></button>
              </div>
            </div>
          </div>
            ...
            <div style={{ display: "flex", flexWrap: "wrap", gap: 6, marginBottom: 10 }}>
              {(detail.blockedDates || []).map((bd: any) => (
                <div key={bd._id} style={{ display: "flex", alignItems: "center", gap: 4, background: "rgba(244,63,94,0.1)", border: "1px solid rgba(244,63,94,0.3)", borderRadius: 8, padding: "4px 8px", fontSize: 12 }}>
                  {bd.date}
                  <button onClick={async () => { try { await removeBlockedDate({ blockedDateId: bd._id }); toast.success("Date unblocked"); } catch(e: any) { toast.error(e.message); } }} style={{ background: "none", border: "none", color: "#fb7185", cursor: "pointer", padding: 0, marginLeft: 2 }}><X size={10} /></button>
                </div>
              ))}
            </div>
            <div style={{ display: "flex", gap: 8 }}>
