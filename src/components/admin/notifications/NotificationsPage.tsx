...
import { Send, Users, User, Store, Bell } from "lucide-react";
    ...
    <div>
      <div className="admin-page-header">
        ...
        <button className="admin-btn admin-btn-primary" onClick={() => setShowCompose(!showCompose)}>
          <Send size={14} /> {showCompose ? "Hide Compose" : "Compose Broadcast"}
        </button>
      </div>
        ...
        <div className="admin-card" style={{ marginBottom: 24 }}>
          ...
          <div style={{ marginTop: 16, display: "flex", gap: 10 }}>
            <button className="admin-btn admin-btn-primary" onClick={handleSend} disabled={sending}>
              <Send size={13} /> {sending ? "Sending…" : `Send to ${targetRole === "all" ? "All Users" : targetRole === "customer" ? "Customers" : "Shop Owners"}`}
            </button>
      ...
      <div className="admin-chart-grid admin-chart-grid-2" style={{ marginBottom: 24 }}>
        ...
        <div className="admin-card">
          <div className="admin-card-title" style={{ marginBottom: 12 }}>🏷️ Types</div>
          <ResponsiveContainer width="100%" height={160}>
