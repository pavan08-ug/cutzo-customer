...
export default function CustomerAuthModal({ open, onClose, onAuthenticated, initialLocation, initialGpsLocation }: Props) {
  ...
  useEffect(() => {
    ...
    if (existingUser) {
      const userRecord: CustomerRecord = {
        ...
        location: existingUser.location && existingUser.location !== "Location pending" ? existingUser.location : (initialLocation || "Location pending"),
        gpsLocation: existingUser.gpsLocation || initialGpsLocation,
      ...
      setSetupDraft((current) => ({
        ...
        name: name,
        email,
        location: current.location || initialLocation || "",
        gpsLocation: current.gpsLocation || initialGpsLocation || "",
        authProvider: "google",
      }));
      setStep("setup");
      hideLoading();
  ...
  useEffect(() => {
    if (!open) {
      resetFlow();
    } else if (initialLocation || initialGpsLocation) {
      setSetupDraft((current) => ({
        ...current,
        location: current.location || initialLocation || "",
        gpsLocation: current.gpsLocation || initialGpsLocation || "",
  ...
  }, [open, initialLocation, initialGpsLocation]);
    ...
    if (Capacitor.isNativePlatform()) {
      try {
        ...
        if (errMsg.includes("No credentials available")) {
          setErrorMessage(
            "No Google accounts found on this device, or Google Play Services is missing. Please add a Google account in Android Settings and try again."
          );
        } else {
  ...
  const completeSetup = async () => {
    ...
    try {
      const uid = firebaseUid || existingUser?.uid || "";
      const name = setupDraft.name.trim();
      const location = setupDraft.location.trim() || initialLocation || "Location pending";
            ...
            <>
              ...
              <label className="flex items-start gap-3 mt-1 mb-2">
                ...
                <span className="text-xs font-medium text-muted-foreground leading-snug">
                  I agree to the <span onClick={() => openExternalUrl("/terms.html")} className="font-bold text-foreground hover:underline cursor-pointer">Terms of Service</span> & <span onClick={() => openExternalUrl("/privacy.html")} className="font-bold text-foreground hover:underline cursor-pointer">Privacy Policy</span>
                </span>
              </label>
