...
function ShopCard({ shop, onSelect }: { shop: Shop; onSelect: () => void }) {
  ...
  return (
    <div
      ...
      onClick={onSelect}
    >
      <div className="relative h-48 overflow-hidden rounded-t-[16px]" ref={images.length > 1 ? emblaRef : undefined}>
        {images.length > 1 ? (
...
export default function HomeScreen({ onShopSelect, onNavigate, onLogout, customer }: Props) {
  ...
  // Use a single persistent query so the subscription never resets when GPS resolves
  const allShopsQuery = useQuery(api.shops.getShops);
  const convexShops = allShopsQuery ?? [];
  const loading = allShopsQuery === undefined;
  ...
  const shops: Shop[] = convexShops.map((s: any) => {
    const relationalServices: any[] = s.services ?? [];
    const distanceVal = s.locationLabel ?? s.address ?? "Nearby";
    return {
      id: s._id as string,
      ownerId: s.ownerId,
  ...
  return (
    <div className="flex h-screen flex-col bg-background">
      ...
      {/* Sticky Header */}
      <div className="sticky top-0 z-20 bg-white shadow-[0_2px_10px_rgba(0,0,0,0.06)] pb-[8px] pt-[8px] px-4 flex-none safe-top">
        <div className="flex items-center justify-between">
          <div className="flex flex-col items-start justify-center">
            <h1 className="font-montserrat text-[13px] font-bold uppercase tracking-wider text-primary leading-tight whitespace-nowrap">
              India's Barber Booking App
            </h1>
          </div>
          
          <div className="text-right flex justify-end">
            <img src="/cutzo-homescreen.png" alt="CUTZO" className="h-14 w-auto object-contain" />
          </div>
        </div>
      </div>
