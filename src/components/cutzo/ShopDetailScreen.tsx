...
function getOptimizedImageUrl(url: string, width = 600, quality = 80) {
  if (!url) return "";
...
export default function ShopDetailScreen({ shop, onBack, onBookNow, onOpenNotifications }: Props) {
  ...
  const dbReviewsRaw = useQuery(
    api.reviews.getShopReviews,
    isValidShopId ? { shopId: shop.id as Id<"shops"> } : "skip"
  );
  ...
  const barberStatus = useQuery(
    api.walkIns.getBarberStatusByShopId,
    isValidShopId ? { shopId: shop.id as Id<"shops"> } : "skip"
  );
              ...
              <div className="flex items-center gap-2">
                ...
                <div>
                  ...
                  <p className="text-xs font-bold text-foreground">
                    {(shop.distance ?? "").startsWith("0.0") || shop.distance === "0 km" ? "Nearby" : shop.distance}
                  </p>
                </div>
              </div>
