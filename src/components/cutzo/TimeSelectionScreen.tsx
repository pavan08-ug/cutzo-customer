...
interface Props {
  ...
  totalPrice: number;
  onBack: () => void;
  onContinue: (date: string, time: string, preferredBarber?: string) => void;
}
...
export default function TimeSelectionScreen({
  ...
  const isValidShopId = typeof shopId === "string" && shopId.length > 5 && shopId !== "undefined" && shopId !== "null" && !shopId.startsWith("shop-");

  // ── Convex queries
  const availability = useQuery(
    api.shops.getShopAvailability,
    isValidShopId ? { shopId: shopId as Id<"shops"> } : "skip"
  );
  ...
  return (
    <div className="flex flex-col bg-muted" style={{ height: "100dvh" }}>
      ...
      <div className="flex-1 overflow-y-auto px-4 pt-4 pb-36 space-y-4">
        ...
        <div className="rounded-[20px] bg-card p-4 card-shadow border border-border/60">
          <div className="flex items-center justify-between mb-3">
            ...
            <span className="text-[10px] font-bold px-2.5 py-0.5 rounded-full bg-primary/10 text-primary">
              {shop?.enablePreferredBarber || (shop?.barbers && shop.barbers.length > 0) ? "Optional Choice" : "Auto-Assigned"}
            </span>
          </div>

          <div className="flex flex-wrap gap-2">
            {/* Default: First Available Barber */}
            <button
              onClick={() => setSelectedBarber("First Available Barber")}
              className="flex items-center gap-2 px-4 py-2.5 rounded-[14px] text-xs font-bold transition-all scale-tap"
              style={{
                background: selectedBarber === "First Available Barber"
                  ? "linear-gradient(135deg, #8F00FF 0%, #5F00CC 100%)"
                  : "hsl(var(--muted))",
                color: selectedBarber === "First Available Barber" ? "#fff" : "hsl(var(--foreground))",
                boxShadow: selectedBarber === "First Available Barber" ? "0 4px 12px rgba(143,0,255,0.3)" : "none",
                border: selectedBarber === "First Available Barber" ? "1.5px solid transparent" : "1.5px solid hsl(var(--border))"
              }}
            >
              <Sparkles className="h-3.5 w-3.5" />
              <span>First Available Barber (Fastest)</span>
            </button>

            {/* Keep the option for the customer to choose a preferred barber ONLY if the shop has that feature enabled */}
            {(shop?.enablePreferredBarber || (shop?.barbers && shop.barbers.length > 0)) && (
              (shop?.barbers || ["Alex (Master Stylist)", "David (Senior Barber)", "Sam (Color Specialist)"]).map((barberName) => {
                const isSel = selectedBarber === barberName;
                return (
                  <button
                    key={barberName}
                    onClick={() => setSelectedBarber(barberName)}
                    className="flex items-center gap-2 px-3.5 py-2.5 rounded-[14px] text-xs font-semibold transition-all scale-tap"
                    style={{
                      background: isSel ? "linear-gradient(135deg, #8F00FF 0%, #5F00CC 100%)" : "hsl(var(--card))",
                      color: isSel ? "#fff" : "hsl(var(--foreground))",
                      boxShadow: isSel ? "0 4px 12px rgba(143,0,255,0.3)" : "0 1px 3px rgba(0,0,0,0.05)",
                      border: isSel ? "1.5px solid transparent" : "1.5px solid hsl(var(--border))"
                    }}
                  >
                    <span>✂️</span>
          ...
          {!(shop?.enablePreferredBarber || (shop?.barbers && shop.barbers.length > 0)) && (
            <p className="mt-2.5 text-[11px] text-muted-foreground font-medium leading-relaxed">
              ℹ️ To ensure minimal wait time, this shop automatically routes your booking to the first available professional.
            </p>
          )}
        </div>
          ...
          {availability === undefined ? (
            ...
            <div className="flex gap-1 items-stretch">
              ...
              <WheelPicker
                ...
                label="Date"
                className="flex-[1.8] min-w-0"
                renderItem={(idx) => (
                  <div className="text-center px-1 w-full flex flex-col items-center justify-center">
                    <div className="text-[14px] sm:text-[15px] font-bold leading-tight tracking-tight">
              ...
              <div className="flex items-center justify-center text-2xl font-black text-muted-foreground pt-5 self-center">
                :
              </div>
      ...
      >
        <button
          onClick={() => onContinue(selectedDate, time24, selectedBarber)}
