...
export function formatError(error: any): string {
  ...
  // 1. Direct structured userMessage from ConvexError
  if (error.data && typeof error.data === "object" && typeof error.data.userMessage === "string") {
    return error.data.userMessage;
  }

  let message = typeof error === "string" ? error : error.message || "An unexpected error occurred.";

  // 2. Extract userMessage if embedded in stringified ConvexError dump
  if (typeof message === "string" && message.includes('"userMessage":')) {
    try {
      const match = message.match(/\{.*?"userMessage"\s*:\s*"([^"]+)".*?\}/);
      if (match && match[1]) {
        return match[1];
      }
    } catch (e) {
