...
import { clearCustomerSession, getActiveCustomer, saveCustomer } from "@/components/cutzo/authStorage";
      ...
      return (
        <div style={{
          ...
          <p style={{ fontSize: "14px", color: "#f87171", fontWeight: 600 }}>
            {this.state.error.name}: {formatError(this.state.error.message)}
          </p>
          <div style={{
            ...
            {formatError(this.state.error.stack || "No stack trace available")}
          </div>
          <button
...
function AppInner() {
  ...
  const [selectedBarber, setSelectedBarber] = useState<string>("First Available Barber");
  ...
  // Automatically fetch and update user location after login / app load
  useEffect(() => {
    if (customer) {
      const fetchAndUpdateLocation = async () => {
        try {
          ...
          const position = await Geolocation.getCurrentPosition({ enableHighAccuracy: false, maximumAge: 3600000 });
          ...
          if (city && city !== customer.location) {
            ...
            setCustomer(prev => prev ? { ...prev, location: city, gpsLocation: `${latitude},${longitude}` } : null);
            // Also update localStorage so it persists across reloads
            const updatedCustomer = { ...customer, location: city, gpsLocation: `${latitude},${longitude}` };
  ...
  // SplashScreen calls this when both the visual sequence AND all
  // animation assets are fully preloaded into device memory.
  ...
  // Safety timer to guarantee the app auto-advances from splash screen
  useEffect(() => {
    if (screen === "splash") {
  ...
  const customerBookings: any[] = (convexBookingsRaw ?? []).map((b: any) => ({
    ...
    service: b.service,
    services: b.services,
    preferredBarber: b.preferredBarber || b.barber,
    date: b.date,
    time: b.time,
    address: b.address ?? "",
    price: b.price,
    ...
    expectedStartTime: b.expectedStartTime,
  ...
  const reviewedBookingIds = new Set<string>((reviewedBookingIdsRaw as string[]) || []);

  const isValidShopId = (id: any): id is Id<"shops"> => {
    return typeof id === "string" && id.length > 5 && id !== "undefined" && id !== "null" && !id.startsWith("shop-");
  };

  const today = new Date().toISOString().split("T")[0];
  const dbBookedSlots = useQuery(
    api.shops.getShopBookedSlots,
    selectedShop && isValidShopId(selectedShop.id) ? { shopId: selectedShop.id, fromDate: today } : "skip"
  );
  ...
  const fullShopDetails = useQuery(
    ...
    selectedShop && isValidShopId(selectedShop.id) ? { shopId: selectedShop.id } : "skip"
  );
  ...
  return (
    ...
    >
      <div className="customer-theme w-full min-h-screen">
          ...
          {screen === "value" && (
            <ValueScreen onGetStarted={handleOpenCustomer} />
          )}
          ...
          {screen === "timeSelect" && effectiveShop && (
            <TimeSelectionScreen
              ...
              onContinue={(date, time, barber) => {
                setSelectedDate(date);
                setSelectedTime(time);
                setSelectedBarber(barber || "First Available Barber");
                ...
                try {
                  ...
                  const { bookingId, otp } = await createBooking({
                    ...
                    services: booking.services.map(s => ({
                      ...
                      duration: typeof s.duration === "string" ? parseInt(s.duration) || 30 : (Number(s.duration) || 30)
                    })),
                    totalAmount: booking.services.reduce((acc, s) => acc + (Number(s.price) || 0), 0),
                    date: booking.date,
                    time: booking.time,
                    clientNow: Date.now(),
                  });
                  ...
                  toast.error(formatError(error) || "Failed to book slot. It might be full already.");
                }
              }}
            />
          )}
            ...
            <ActivityScreen
              ...
              onCancelBooking={async (bookingId) => {
                ...
                try {
                  ...
                  toast.error(formatError(err) || "Failed to cancel booking.");
                }
              }}
              onRescheduleBooking={async (bookingId, newDate, newTime) => {
                ...
                try {
                  ...
                  toast.error(formatError(err) || "Failed to reschedule booking.");
                }
              }}
              onRebook={(booking) => {
                const rebookServices: Service[] = (booking.services && booking.services.length > 0)
                  ? booking.services.map((s: any) => ({
                      id: s.id || `svc-${Math.random()}`,
                      name: s.name || booking.service,
                      price: s.price || booking.price,
                      duration: typeof s.duration === "number" ? `${s.duration} min` : (s.duration || "30 min"),
                      icon: "Scissors",
                    }))
                  : booking.service
                    ? booking.service.split(",").map((name, idx, arr) => ({
                        id: `svc-${idx}`,
                        name: name.trim(),
                        price: Math.round(booking.price / arr.length) || 100,
                        duration: "30 min",
                        icon: "Scissors",
                      }))
                    : [{ id: "svc-default", name: "Haircut & Grooming", price: booking.price || 150, duration: "30 min", icon: "Scissors" }];

                const rebookShop: Shop = {
                  id: booking.shopId,
                  ownerId: booking.ownerId || "",
                  name: booking.shopName,
                  image: booking.shopImage || "/placeholder.svg",
                  ...
                  distance: "",
                  locationLabel: "",
                  startingPrice: booking.price,
                  nextSlot: "",
                  category: "Salon",
                  tags: [],
                  about: "",
                  openTime: (booking as any).shopOpenTime || "09:00",
                  closeTime: (booking as any).shopCloseTime || "21:00",
                  services: rebookServices,
                  availabilitySlots: [],
                  blockedDates: [],
                };

                setSelectedShop(rebookShop);
                setSelectedServices(rebookServices);
                navigateTo("timeSelect");
              }}
            />
          )}
          ...
          {screen === "terms" && (
            <TermsScreen onBack={() => navigateTo("profile", "back")} />
          )}
